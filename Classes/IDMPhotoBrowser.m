...
- (IDMAbstractZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index;
- (IDMAbstractZoomingScrollView *)pageDisplayingPhoto:(id<IDMPhoto>)photo;
- (IDMAbstractZoomingScrollView *)dequeueRecycledPage;
- (void)configurePage:(IDMAbstractZoomingScrollView *)page forIndex:(NSUInteger)index;
...
+(CGRect)fullScreenRectForImage:(UIImage*)imageFromView {
    CGRect screenBound = [[UIScreen mainScreen] bounds];
    CGFloat screenWidth = screenBound.size.width;
    CGFloat screenHeight = screenBound.size.height;
    float scaleFactorW = (imageFromView ? imageFromView.size.width : screenWidth) / screenWidth;
    float scaleFactorH = (imageFromView ? imageFromView.size.height : screenHeight) / screenHeight;
    float finalH = imageFromView.size.height / scaleFactorW;
    CGRect finalImageViewFrame;
    if (finalH < screenHeight) {
        finalImageViewFrame = CGRectMake(0, (screenHeight/2)-(finalH/2), screenWidth, finalH);
    } else {
        float finalW = imageFromView.size.width / scaleFactorH;
        finalImageViewFrame = CGRectMake((screenWidth/2)-(finalW/2), 0, finalW, screenHeight);
    }
...
- (void)performPresentAnimation {
    ...
    CGRect finalImageViewFrame = [IDMPhotoBrowser fullScreenRectForImage:imageFromView];
...
- (void)performCloseAnimationWithScrollView:(IDMAbstractZoomingScrollView*)scrollView {
...
- (void)viewWillLayoutSubviews {
    ...
    for (IDMAbstractZoomingScrollView *page in _visiblePages) {
...
- (void)loadAdjacentPhotosIfNecessary:(id<IDMPhoto>)photo {
    IDMAbstractZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
...
- (void)handleIDMPhotoLoadingDidEndNotification:(NSNotification *)notification {
    ...
    IDMAbstractZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    [page display];
    [self loadAdjacentPhotosIfNecessary:page.photo];
}
...
- (void)tilePages {
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            // Add new page
            IDMAbstractZoomingScrollView *page;
            
            page = [[_pageClass alloc] initWithPhotoBrowser:self];
...
- (IDMAbstractZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
    IDMAbstractZoomingScrollView *thePage = nil;
    for (IDMAbstractZoomingScrollView *page in _visiblePages) {
...
- (void)configurePage:(IDMAbstractZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    //    __weak IDMAbstractZoomingScrollView* weakPage = page;
...
- (IDMAbstractZoomingScrollView *)dequeueRecycledPage {
    IDMAbstractZoomingScrollView *page = [_recycledPages anyObject];
    if (page) {
